...
export default function SitesClient({ sites }: { sites: SiteRow[] }) {
  ...
  const filtered = sites.filter(s => {
    const matchSearch = !search || s.id.toLowerCase().includes(search.toLowerCase()) ||
      (s.active?.engineer_name ?? '').toLowerCase().includes(search.toLowerCase())
  ...
  return (
    <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', overflow: 'hidden', display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
      ...
      {/* Filters — fixed */}
      <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border)', display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0 }}>
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search site ID or engineer…" style={{ ...inp, width: 220 }} />
      ...
      <div style={{ overflowX: 'auto', overflowY: 'auto', flex: 1 }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {filtered.length === 0 && (
              <tr><td colSpan={7} style={{ ...td, textAlign: 'center', color: 'var(--text3)', padding: 40 }}>No sites found</td></tr>
            )}
            {filtered.map(s => (
              <tr key={s.id}>
                <td style={td}><Badge variant="site">{s.id}</Badge></td>
                <td style={td}><Badge variant={s.active ? 'out' : 'in'}>{s.active ? 'Out' : 'Available'}</Badge></td>
                <td style={{ ...td, color: s.active ? 'var(--text)' : 'var(--text3)' }}>
                  {s.active ? s.active.engineer_name : '—'}
                </td>
                <td style={{ ...td, fontSize: 12, color: 'var(--text2)' }}>
                  {s.active?.engineer_company || '—'}
                </td>
