...
export default function Topbar({ title, sub }: { title: string; sub?: string }) {
  ...
  useEffect(() => {
    const tick = () => {
      const d = new Date()
      setNow(d.toLocaleString('en-AU', { weekday: 'short', day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit', hour12: true }))
    }
    tick()
  ...
  return (
    <>
      ...
      {showModal && <CheckoutModal onClose={() => setShowModal(false)} onSuccess={() => { setShowModal(false); window.location.reload() }} />}
    </>
  )
}
